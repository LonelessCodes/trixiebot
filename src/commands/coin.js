...
module.exports = function install(cr) {
    cr.registerCommand(
        "coin",
        new SimpleCommand(async ({ content, ctx }) => {
            ...
            if (!coin.includes(bet)) {
                await ctx.send(
                    new Translation("coin.invalid", "`{{bet}}` isn't a valid side of le coin. `heads` or `tails`?!", { bet })
                );
                return;
            }
        ...
        .setHelp(new HelpContent().setUsage("<?bet>").addParameterOptional("bet", "your bet. Either `heads` or `tails`"))
