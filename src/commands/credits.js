...
const CommandPermissions = require("../util/commands/CommandPermission").default;
...
module.exports = function install(cr) {
    const bankCmd = cr
        .registerCommand("bank", new TreeCommand())
        .setHelp(
            new HelpContent()
                .setDescription(
                    "Trixie's own currency system so you can send, receive, spend, earn credits for additional features or rewards."
                )
                .setUsage("", "Look at your bank account")
        )
        .setCategory(Category.ECONOMY);

    bankCmd.registerDefaultCommand(
        new SimpleCommand(async ({ guild, author, prefix, ctx }) => {
            const account = await credits.getAccount(author);
            if (!account) {
                return new Translation(
                    "bank.no_account",
                    "It looks like you haven't opened a bank account yet. How about doing so with `{{prefix}}bank create`",
                    { prefix }
                );
            }

            const embed = basicTEmbed(new Translation("bank.account", "Bank Account"), author);
            ...
            const trans_raw = await credits.getTransactions(author, 5);

            const locale = await ctx.translator();

            if (trans_raw.length > 0) {
                const transactions = [];
                for (const trans of trans_raw) {
                    transactions.push({
                        ts: moment(trans.ts).locale(locale.locale).fromNow(),
                        cost: (trans.cost >= 0 ? "+" : "") + ResolvableObject.resolve(new NumberFormat(trans.cost), locale),
                ...
                for (const trans of transactions) {
                    trans_str.push(
                        trans.ts +
                            new Array(ts_length - trans.ts.length).fill(" ").join("") +
                            " | " +
                            trans.cost +
                            new Array(cost_length - trans.cost.length).fill(" ").join("") +
                            " | " +
                            trans.description
                    );
                }
    ...
    bankCmd
        .registerSubCommand(
            ...
            new SimpleCommand(async context => {
                ...
                if (account.exists) {
                    const str = new TranslationMerge(
                        new Translation("bank.already_have_account", ":atm: You have already created a bank account!")
                    );
                    if (account.account.balance > 0)
                        str.push(
                            new Translation("bank.already_balance", "You even already have **{{money}}**!", {
                                money: credits.getBalanceTrans(account.account.balance, await credits.getName(context.guild)),
                            })
                        );
                ...
                return new Translation(
                    ...
                    ":atm: Ayy you now have a bank account! Check it out at `{{prefix}}bank`",
                    { prefix: context.prefix }
                );
            })
        )
        .setHelp(new HelpContent().setUsage("", "Create a bank account to send, receive, spend and earn credits."));
        ...
        .registerSubCommand(
            ...
            new SimpleCommand(async context => {
                ...
                if (!account) {
                    return new Translation(
                        "bank.create_first",
                        "Before you can use any money related activities, please create a bank account using `{{prefix}}bank create`",
                        { prefix: context.prefix }
                    );
                }

                return new Translation("bank.curr_balance", ":yen: You currently have an account balance of **{{money}}**. oof", {
                    money: credits.getBalanceTrans(account.balance, await credits.getName(context.guild)),
                });
            })
        )
        .setHelp(new HelpContent().setUsage("", "Check out the balance on your account!"));

    bankCmd
        .registerSubCommand(
            "pay",
            new SimpleCommand(async ({ message, prefix, content }) => {
                ...
                const other_user = message.mentions.users.first();
                if (!other_user) {
                    return new Translation(
                        "bank.pay_help",
                        "To pay someone money, you need to tell me *who* I should pay it to. Remember it's `<@user> <amount>`"
                    );
                }
                ...
                try {
                    ...
                    return new Translation(
                        "bank.pay_invalid_num",
                        "That is not a valid number, bruh! Remember it's `<@user> <amount>`"
                    );
                }
                ...
                if (amount < 10) {
                    return new Translation("bank.pay_minimum", "You can only pay someone at least {{money}}", {
                        money: credits.getBalanceTrans(10, name),
                    });
                }
                ...
                if (!(await credits.canPurchase(me, amount))) {
                    return new Translation(
                        "bank.pay_not_enough",
                        ":atm: You do not have enough money on your account to pay {{user}} this much",
                        { user: userToString(other_user) }
                    );
                }

                await credits.makeTransaction(
                    message.guild,
                    other_user,
                    amount,
                    "pay",
                    `Got money from ${userToString(me, true)}`
                );
                await credits.makeTransaction(
                    message.guild,
                    me,
                    -amount,
                    "pay",
                    `Sent money to ${userToString(other_user, true)}`
                );

                return new TranslationFormatter(`💴 **{{money}}**\n${userToString(me)} ▶ ${userToString(other_user)}`, {
                    money: credits.getBalanceTrans(amount, name),
                });
            })
        )
        .setHelp(
        ...
        .registerSubCommand(
            ...
            new SimpleCommand(async context => {
                ...
                if (trans_raw.length > 0) {
                    ...
                    await new PaginatorAction(
                        new Translation("bank.transactions", "Transactions"),
                        new Translation("bank.transactions_desc", "All of {{user}}'s (your) transactions", {
                    ...
                    return new Translation(
                        "bank.trans_no_money_yet",
                        "Looks like you didn't earn or spend money yet! Let's start by `{{prefix}}daily`, to earn some munz",
                        { prefix: context.prefix }
                    );
                }
            })
        )
        .setHelp(new HelpContent().setUsage("", "Display all your transactions from ever!"));
    bankCmd.registerSubCommandAlias("transactions", "trans");

    // redo this
    bankCmd
        .registerSubCommand("setname", new OverloadCommand())
        .registerOverload(
            "0",
            new SimpleCommand(async context => {
                ...
                return new TranslationMerge(
                    ...
                    "`" + name + "`",
                    new Translation("bank.name.example", "Example: **{{example}}**", {
                        example: credits.getBalanceTrans(Math.floor(Math.random() * 50), name),
                    })
                ).separator("\n");
            })
        )
        .registerOverload(
            ...
            new SimpleCommand(async ({ guild, content }) => {
                if (content.length > 100)
                    return new Translation(
                        "bank.name.too_long",
                        "The name of your this currency should be shorter than 100 character"
                    );
                if (/`/g.test(content))
                    return new Translation(
                        "bank.name.forbidden_chars",
                        "That name contains forbidden characters that would break me :'c"
                    );
        ...
        .setHelp(
            new HelpContent()
                .setUsage(
                    "<?name>",
                    "Change the name of the currency. If not given any arguments, view the current configuration"
                )
    ...
    bankCmd
        .registerSubCommand("set", new OverloadCommand())
        .registerOverload(
            "1+",
            new SimpleCommand(async ({ message, content }) => {
                ...
                const new_balance = await credits.makeTransaction(
                    message.guild,
                    member,
                    amount - old_balance,
                    "set_balance",
                    "Balance was set to " +
                        credits.getBalanceString(amount, await credits.getName(message.guild)) +
                        " by " +
                        userToString(message.author, true)
                );
    ...
    cr.registerCommand(
        ...
        new SimpleCommand(async context => {
            ...
            if (time_left > 0) {
                const m = await context.send(
                    new Translation("bank.daily_too_soon", ":atm: {{user}}, daily :yen: credits reset in **{{time_left}}**.", {
                        user: userToString(user),
                        time_left: toHumanTime(time_left),
                    })
                );
            ...
            const str = new TranslationMerge(
                new Translation("bank.daily", ":atm: {{user}}, you received your :yen: **{{money}}**!", {
                    user: userToString(user),
                    money: credits.getBalanceTrans(dailies, currency_name, "daily"),
                })
            );
            ...
            if (bonus > 0)
                str.push(
                    new Translation(
                        "bank.daily_streak",
                        "You completed a streak and added an extra :yen: **{{bonus}}** (**{{total}}** total)!",
                        { bonus: credits.getBalanceTrans(bonus, currency_name, "bonus"), total: new NumberFormat(total) }
                    )
                );
        ...
        .setHelp(new HelpContent().setUsage("", "Gather your daily payouts every day and occasionally get bonus money."))
        .setCategory(Category.ECONOMY);
