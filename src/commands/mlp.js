...
module.exports = function install(cr) {
    cr.registerCommand("mlp", new OverloadCommand())
        .registerOverload(
            "1+",
            new SimpleCommand(async ({ content }) => {
                const searchRequest = await fetch(
                    `http://mlp.wikia.com/api/v1/Search/List?query=${encodeURIComponent(content)}&format=json&limit=1`
                );
                ...
                const abstractRequest = await fetch(
                    `http://mlp.wikia.com/api/v1/Articles/Details?ids=${id}&format=json&abstract=500`
                );
                /**
                ...
                if (info.toArray().length) {
                    title = info
                        ...
                        .first()
                        .text()
                        .replace(info.find("tr > th").first().find("span").text() || "", str => ` ${str}`);
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(
                        abstract.abstract
                            ...
                            .replace(/ *\[[\w ]+\] */gi, "")
                    ) // get rid of reference squared brackets
                    .setURL(pageURL)
                    .setFooter(
                        `type: ${abstract.type} | ${abstract.comments} ${
                            abstract.comments === 1 ? "comment" : "comments"
                        } | Quality of the result: ${item.quality}%`
                    );
                ...
                if (abstract.thumbnail) {
                    if (info.toArray().length) {
                        ...
                        if (thumb.toArray().length) {
                            embed.setThumbnail(thumb.data("src"));
                        } else {
                ...
                for (const field of fields) embed.addField(field.key, field.value, true);
        ...
        .setHelp(
            new HelpContent()
                .setDescription(
                    "Query the MLP Wikia for fun! Everything there: ALL the ponies, ALL the episodes, ALL the places."
                )
