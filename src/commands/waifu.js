...
module.exports = function install(cr, { db }) {
    ...
    waifuCommand.registerSubCommand("claim", new SimpleCommand(async ({ message, content }) => {
        const {
            mentioned_member,
            owner_waifus,
            slots,
        ...
        if (slots - owner_waifus.length === 0) {
            return new Translation("waifu.slots_full", "Stop it. Get some help. You have filled all your waifu slots already!");
        }
        ...
        if (owner) {
            ...
            if (owner_member) {
                return new Translation("waifu.claim.already_claimed", "Oh nu. {{user}} already claimed this beautiful user!", { user: userToString(owner_member) });
            }
    ...
    waifuCommand.registerSubCommand("steal", new SimpleCommand(async ({ message, prefix, content, ctx }) => {
        const {
            mentioned_member,
            owner_waifus,
            all_waifus,
        ...
        if (steal_cooldown_guild.includes(message.guild.id)) {
            return new Translation("waifu.steal.cooldown_global", "Whoa whoa not so fast! There's also a little global cooldown.");
        }
        ...
        if (steal_cooldown_user.includes(message.author.id)) {
            return new Translation("waifu.steal.cooldown", "Whoa whoa not so fast! Doing this too often in a row will obvi defeat the purpose of the claim command");
        }
        ...
        if (!mentioned_member) {
            ...
            if (w) {
                ...
                if (owner_member) {
                    ...
                    return new Translation("waifu.steal.not_claimed_yet", "This waifu has not been claimed yet. Use `{{prefix}}waifu claim @ {{user}}` to call them your own!", { prefix, user: mentioned_member.displayName });
                }
            } else {
                return new Translation("waifu.steal.not_claimed_yet", "This waifu has not been claimed yet. Use `{{prefix}}waifu claim @ {{user}}` to call them your own!", { prefix, user: mentioned_member.displayName });
            }
        ...
        const m1 = await ctx.send(new Translation("waifu.steal.story", "{{owner}}'s waifu {{waifu}} just seemed to have appeared nearby. You are quietly approaching them...", { owner: userToString(ownerUser), waifu: userToString(waifuUser) }));
        await timeout(1000);
        ...
        if (random > 95) {
            ...
            await ctx.edit(m2, new Translation("waifu.steal.failed", "{{user}} had got wind of your plans and dashed off at the next chance :c", { user: userToString(waifuUser) }));
            await timeout(60000);
            await m1.delete().catch(() => { /* Do nothing */ });
        }
    })).setHelp(new HelpContent()
    ...
    waifuCommand.registerSubCommand("escape", new SimpleCommand(async ({ message, ctx }) => {
        const {
            owner_of_me,
        ...
        if (escape_cooldown_user.includes(message.author.id)) {
            return new Translation("waifu.escape.cooldown", "Whoa whoa not so fast! Doing this too often in a row will obvi defeat the purpose of this all");
        }
        ...
        const m1 = await ctx.send(new Translation("waifu.escape.story", "You are out for walkies with your owner, {{owner}}. You are following them and their orders, preparing for the perfect escape...", { owner: userToString(ownerUser) }));
        await timeout(1000);
        ...
        if (random > 95) {
            ...
            await ctx.send(new Translation("waifu.escape.failed", "{{user}} reacted quickly and grabs you by your arm. \"One day...\" you mumble to yourself.", { user: userToString(ownerUser) }));
            await timeout(60000);
            await m1.delete().catch(() => { /* Do nothing */ });
            await m2.delete().catch(() => { /* Do nothing */ });
        }
    ...
    waifuCommand.registerSubCommand("buyslot", new SimpleCommand(async ({ message, ctx }) => {
        const user = message.author;
        if (buy_active.has(user.id)) return;

        if (buy_cooldown_user.test(user.id)) {
            return new Translation("waifu.buy.cooldown", "You can only purchase 2 slots an hour! Wait {{time}} before you can go again", { time: toHumanTime(buy_cooldown_user.tryAgainIn(user.id)) });
        }

        const { slots } = await getData(message, null, database, databaseSlots);

        if (slots >= MAX_SLOTS) {
            return new Translation("waifu.buy.max_slots", "You have reached the maximum amount of waifu slots, which is {{slots}}!", { slots: MAX_SLOTS });
        }
    ...
    waifuCommand.registerDefaultCommand(new SimpleCommand(async ({ message, prefix }) => {
        const {
            owner_waifus,
            owner_of_me,
            slots,
        ...
        if (!owner_waifus.length) {
            embed.setDescription(new Translation("waifu.no_waifus", "No waifus to see here, just dust :(\nYou can get waifus by claiming someone! Use `{{prefix}}waifu claim` to get started.", { prefix }));
        } else {
            let str = "";
        ...
        embed.setFooter(new Translation("waifu.footer", "Total Waifus: {{waifus}} - Available Slots: {{slots}}", { waifus: owner_waifus.length, slots: slots - owner_waifus.length }));
