...
class CCManager {
    constructor(client, database) {
        ...
        this.link = Comlink.wrap(childProcessAdapter(this.fork));
        Comlink.expose(new WorkerMethods(client, timer), childProcessAdapter(this.fork));
    ...
    async run(message, opts) {
        ...
        let error;
        try {
            response = await this.link.run(opts);
        } catch (err) {
            error = err;
        }
    ...
    async compile(code) {
        const { errors, cst } = await this.link.compile(code);
        return { errors, cst };
    }

    async lint(code) {
        const { errors } = await this.link.lint(code);
        return { errors };
    }
