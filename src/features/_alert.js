...
const twitchClient = TwitchClient.withCredentials(keys.twitch.client_id);
...
module.exports = async function install(cr, client, config, db) {
    ...
    // /** @type {Channel[]} */
    // const onlineP = new Array;
    // /** @type {Channel[]} */
    // const onlineT = new Array;
    ...
    //     const stream = database.find({ service: "picarto" });
    //     stream.addListener("data", config => checkChangePicarto(picartoOnline, config));
    //     stream.once("end", () => { });
    //     stream.once("error", err => { log(err); });

    //     const twitchConfig = await database.find({ service: "twitch" }).toArray();
    //     const twitchOnline = await twitchClient.streams.getStreams(twitchConfig.map(c => c.userId));
    //     for (const config of twitchConfig) {
    //         checkChangeTwitch(twitchOnline, config);
    //     }
    // }

    // async function checkChangePicarto(picartoOnline, savedConfig) {
    //     const guild = client.guilds.get(savedConfig.guildId);
    //     if (!guild) return;
    //     if (!guild.available) return;

    //     const oldChannel = onlineP.find(channel => {
    ...
    //     if (!guildChannel) {
    //         if (oldChannel) onlineP.splice(onlineP.indexOf(oldChannel), 1);
    ...
    //             if (oldChannel)
    //                 onlineP.splice(onlineP.indexOf(oldChannel), 1);
    ...
    // async function checkChangeTwitch(twitchOnline, savedConfig) {
    //     const guild = client.guilds.get(savedConfig.guildId);
    //     if (!guild) return;
    //     if (!guild.available) return;

    //     const oldChannel = onlineT.find(channel => {
    //         return savedConfig.service === channel.service &&
    //             savedConfig.userId === channel.userId &&
    //             savedConfig.guildId === channel.guildId;
    //     });
    ...
    //     const guildChannel = guild.channels.get(savedConfig.channelId);
    //     if (!guildChannel) {
    //         if (oldChannel) onlineT.splice(onlineT.indexOf(oldChannel), 1);
    ...
    //     let channelPage = twitchOnline.find(channelPage => savedConfig.userId === channelPage.id);
    //     if (!channelPage) {
    //         // remove the channel from the recently online list                
    //         if (savedConfig.messageId || oldChannel) {
    //             if (oldChannel)
    //                 onlineT.splice(onlineT.indexOf(oldChannel), 1);
    ...
    //         const embed = new Discord.RichEmbed()
    //             .setColor(CONST.COLOR.PRIMARY)
    //             .setURL(channelPage.channel.url)
    //             .setAuthor(channelPage.channel.displayName)
    //             .setTitle(channelPage.channel.status)
    //             .addField("Followers", channelPage.channel.followers, true)
    //             .addField("Total Viewers", channelPage.channel.views, true)
    //             .setThumbnail(channelPage.channel.logo)
    //             .setImage(guildChannel.nsfw ?
    //                 `${channelPage.getPreviewUrl("medium")}?${Date.now()}` :
    //                 channelPage.channel.isMature ?
    //                     "https://66.media.tumblr.com/6c2c27a36111b356b65cf21746b72698/tumblr_p4tu9xcuEv1v9xi8y_og_500.jpg" :
    //                     `${channelPage.getPreviewUrl("medium")}?${Date.now()}`)
    //             .setFooter(`${channelPage.channel.isMature ? "NSFW | " : ""}Game: ${channelPage.game}}`);

    //         const onlineMessage = await guildChannel.sendTranslated("{{user}} is live!", {
    //             user: channelPage.channel.name
    //         }, { embed });
    ...
    //             guildId: savedConfig.guildId,
    //             channelId: savedConfig.channelId,
    //             userId: channelPage.id,
    //             name: channelPage.channel.name,
    //             messageId: onlineMessage.id
    //         });
    //         onlineT.push(newchannel);
