...
class NaughtyCommand extends Command {
    constructor(client, config, db) {
        ...
        this.db = db.collection("santaslist");
    }
    async onmessage(message) {
        if (!message.prefixUsed) return;
        if (!/^(naughty|nice)\b/i.test(message.content)) return;
    ...
    usage(prefix) {
        return `\`${prefix}naughty\` - It's shortly before Christmas again! Have you been good this year, or will your potty mouth on Discord leave you with lump of coal in your stocking on Christmas day? It’s time to find out!`;
    }
}

module.exports = NaughtyCommand;
